...
class VM:
    ...
    def matches_requirements(self, required_config: VMConfig) -> bool:
        ...
        #TODO need to check teh networking stuff. Also the requirements for packages.
        # so how do i do it? not sure yet. base image wont cut it because that means having ties with specific vm types.
        return (
            self.config.cpu >= required_config.cpu and
            self.config.memory >= required_config.memory and
            (required_config.network is None or
             self.config.network == required_config.network)
        )
...
class VMManager:
    ...
    def create_vm(self, vm_config: VMConfig, launcher: Launcher) -> VM:
        ...
        try:
            ...
            logger.info(f"Calling scenario.sh create with script: {scenario_script}")

            #TODO somethig is up with this. the vm files are not generated where i expect them to be.
    ...
    def destroy_vm(self, vm: VM):
        """
        Destroy a VM using the cleanup scr ipt.
...
class TestRunner:
    ...
    def load_scenarios(self) -> List[Scenario]:
        ...
        for scenario_data in data.get('scenarios', []):
            vm_config_data = scenario_data.get('vm_config', {})
            vm_config = VMConfig(
                cpu=vm_config_data.get('cpu', 2),
                memory=vm_config_data.get('memory', 4096),
                network=vm_config_data.get('network'),
                packages=vm_config_data.get('packages', [])
            )
    ...
    def run_test(self, scenario: Scenario) -> bool:
        ...
        #TODO the launcher is selected based on hardware requirements. this might be possible to do without the launcher object? could be part of the vm manager.
    ...
    def run_scenario_worker(self, scenario: Scenario):
        ...
        try:
            ...
            #TODO not sure if i need this lock and the map itself.
    ...
    def run(self):
        ...
        #TODO this might need some work to display it properly.
        # Print summary
        logger.info("=" * 60)
