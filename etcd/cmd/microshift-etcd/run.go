...
func (s *EtcdService) configure(cfg *config.Config) {
	...
	if cfg.Etcd.ClusterState != "" {
		s.etcdCfg.ClusterState = cfg.Etcd.ClusterState
	}
	...
	if cfg.Etcd.InitialCluster != "" {
		s.etcdCfg.InitialCluster = cfg.Etcd.InitialCluster
	}
...
func (s *EtcdService) Run() error {
	...
	// why is this not working? learners cant have their thing?
